<?php
//gaztelera
return [
    //Header
    "Logelak" => "Habitaciones",
    "Erreserbak" => "Reservas",
    "Zerbitzuak" => "Servicios",
    "Kontaktua" => "Contacto",
    "Web Konfigurazioa" => "Configuración Web",
    "Saioa hasi" => "Iniciar sesión",
    "Saioa itxi" => "Cerrar sesión",

    //Footer
    "BBC hotela" => "BBC hotel",
    "Helbidea" => "Dirección",
    "Telefonoa" => "Teléfono",
    "Emaila" => "Correo electrónico",
    "Loturak Azkarrak" => "Enlaces rápidos",
    "jarraitu gaitzazu" => "síguenos",
    "Eskubide guztiak erreserbatuta" => "Todos los derechos reservados",

    //Index
    "Hotela" => "Hotel",
    "Hotelaren Logelak" => "Habitaciones del Hotel",
    "Pertsonen kopurua: " => "Número de personas: ",
    "Gela guztiak" => "Todas las habitaciones",
    "2 pertsonentzat" => "Para 2 personas",
    "3 pertsonentzat" => "Para 3 personas",
    "4 pertsonentzat" => "Para 4 personas",
    "5 pertsonentzat" => "Para 5 personas",
    "6 pertsonentzat" => "Para 6 personas",
    "Bilatu" => "Buscar",
    "Ez da logelarik aurkitu ezaugarri hauekin" => "No se encontraron habitaciones con estas características",
    "Logelaren irudia" => "Imagen de la habitación",
    "Hemen klikatu informazio gehiago lortzeko" => "Haz clic aquí para más información",
    "pertsonentzako" => "personas",
    "gau" => "noche",

    //Erreserbak
    "Hotelaren Erreserbak" => "Reservas del Hotel",
    "Zure erreserbak ikusteko erregistratu zaitez." => "Por favor, inicie sesión para ver sus reservas.",
    "Ez duzu erreserbarik kontu honetan." => "No tienes reservas en esta cuenta.",
    "Erreserba Zenbakia" => "Número de Reserva",
    "Logela Izena" => "Nombre de la Habitación",
    "Bezeroa Izena" => "Nombre del Cliente",
    "Erreserba Eguna" => "Fecha de Reserva",
    "Sarrera Eguna" => "Fecha de Entrada",
    "Irteera Eguna" => "Fecha de Salida",
    "Sarrera Ordua" => "Hora de Entrada",
    "Irteera Ordua" => "Hora de Salida",
    "Iruzkina" => "Comentario",
    "Prezioa" => "Precio",
    "Zure erreserbak editatzeko gure aplikazioa instalatu." => "Instale nuestra app para editar sus reservas.",

    //Logelak
    "Ez da logelarik aurkitu." => "No se encontró ninguna habitación.",
    "Logela" => "Habitación",
    "Ezin dira aukeratu egiten diren datak. Mesedez, hautatu beste data batzuk." => "Las fechas seleccionadas no están disponibles. Por favor, elija otras fechas.",
    "Prezioa" => "Precio",
    "Deskripzioa" => "Descripción",
    "Kapazitatea" => "Capacidad",
    "Ezaugarriak" => "Características",
    "Balkoia" => "Balcón",
    "Sukaldea" => "Cocina",
    "Telebista" => "Televisión",
    "Sofa-Ohea" => "Sofá-Cama",
    "Erreserba egin" => "Hacer una reserva",
    "Sarrera data" => "Fecha de entrada",
    "Irteera data" => "Fecha de salida",
    "Erreserbatu" => "Reservar ahora",

    //Zerbitzuak
    "Hotelaren Zerbitzuak" => "Servicios del Hotel",
    "Ordenatu prezioaren arabera" => "Ordenar por precio",
    "Prezio baxuenetik altuenera" => "Precio de menor a mayor",
    "Prezio altuenetik baxuenera" => "Precio de mayor a menor",
    "Zerbitzuaren argazkia" => "Imagen del servicio",

    //Kontaktuak
    "Hotelaren Informazioa" => "Información del Hotel",
    "Gurekin harremanetan jar zaitez" => "Ponte en contacto con nosotros",
    "Gure sare sozialak" => "Nuestras redes sociales",

    //Web Konfigurazioa
    "Kolore Aldaketa" => "Cambio de Color",
    "Header kolorea" => "Color del encabezado",
    "Footer kolorea" => "Color del pie de página",
    "Gorde" => "Guardar",

    //Login
    "Logeatu" => "Iniciar sesión",
    "Erabiltzaile izena edo pasahitz okerrak." => "Nombre de usuario o contraseña incorrectos.",
    "Erabiltzailea ez da aurkitu." => "Usuario no encontrado.",
    "Erabiltzaile izena" => "Nombre de usuario",
    "Pasahitza" => "Contraseña",
    "Sartu" => "Entrar",
    "Ez daukazula konturik?" => "¿No tienes una cuenta?",
    "Erregistratu hemen" => "Regístrate aquí",

    //Erregistratu
    "Erregistratu" => "Registrarse",
    "Erabiltzailea erregistratuta dago." => "El usuario ya está registrado.",
    "Akats bat gertatu da erregistroa egitean." => "Ocurrió un error durante el registro.",
    "Izena" => "Nombre",
    "Abizena" => "Apellido",
    "Jaiotze eguna" => "Fecha de nacimiento",
    "Kontua duzu?" => "¿Ya tienes una cuenta?",
    "Logeatu hemen" => "Inicia sesión aquí"
];
